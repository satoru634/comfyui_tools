...
```bash
python run_workflow.py --input input.json --workflow sdxl --output result.json
```
...
- `loras`: 使用するワークフローの `config.json` の `loras` に定義したキー名を指定する。0〜4個まで指定可能。
- `prompts.positive` / `prompts.negative`: プロンプト文字列。
- `image_size` (省略可能): 生成する画像のサイズ。省略時は使用するワークフローの `default_image_size` を使用する。
  - `width` / `height`: 整数。512〜2048 の範囲で 8 の倍数であること。

## テンプレートの自動選択

ワークフロー名と入力 JSON の `loras` の個数に応じて、使用するテンプレートを自動で選択する。

| LoRA 個数 | 使用テンプレート |
|---|---|
| 0 | `templates/<workflow_name>/template_lora_0.json` |
| 1 | `templates/<workflow_name>/template_lora_1.json` |
| 2 | `templates/<workflow_name>/template_lora_2.json` |
| 3 | `templates/<workflow_name>/template_lora_3.json` |
| 4 | `templates/<workflow_name>/template_lora_4.json` |
...
1. `config.json` を読み込み、接続先 URL・`default_workflow`・ワークフロー別設定を取得する
2. ワークフロー名を決定する（`--workflow` 指定があればその値、省略時は `default_workflow`）
3. 指定ワークフローの設定（`loras`・`default_image_size`）を `workflows[workflow_name]` から取得する
4. 入力 JSON を読み込む
5. `image_size` が省略されている場合、対象ワークフローの `default_image_size` を使用する
6. LoRA名 → ファイル名・ストレングスを対象ワークフローの `loras` で解決する
7. ワークフロー名と LoRA の個数に応じてテンプレートを自動選択し、プロンプト・LoRA・画像サイズを書き換える
8. `POST /prompt` でワークフローを送信し、`prompt_id` を取得する
9. WebSocket (`ws://host/ws?clientId=<uuid>`) で実行完了またはエラーを監視する
10. 完了後、`GET /history/{prompt_id}` で出力ファイル一覧を取得する
11. `result.json` を出力して終了する
...
```json
{
  ...
  "template": "...templates/sdxl/template_lora_2.json",
  "parameters": {
    "positive": "masterpiece, best quality, 1girl ...",
    "negative": "worst quality, bad quality ...",
    "loras": [
...
| LoRA名が対象ワークフローの `loras` に存在しない | result.json にエラー記録して終了 |
...
| `config.json` に `default_workflow` がない | result.json にエラー記録して終了 |
| `config.json` に `workflows` がない | result.json にエラー記録して終了 |
| 指定したワークフロー名が `workflows` に存在しない | result.json にエラー記録して終了 |
| ワークフローに対応するテンプレートディレクトリが存在しない | result.json にエラー記録して終了 |
...
```bash
# ワークフロー名を明示する場合
python run_workflow.py --input input.json --workflow sdxl --output result.json --config config.json

# ワークフロー名を省略する場合（config.json の default_workflow を使用）
...
```python
# ワークフロー名を明示する場合
runner = WorkflowRunner("config.json", workflow_name="sdxl")
...
# image_size を省略する場合（対象ワークフローの default_image_size を使用）
